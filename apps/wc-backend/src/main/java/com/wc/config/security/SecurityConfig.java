 ...
 *       {@code classpath:keys/wc-mock-public.pem}, paired with the demo
 *       private key in {@code scripts/wc-mock-private.pem}. Mint tokens via
 ...
 * {@code https://wc/roles} claim per Auth0 OIDC convention).
 */
@Configuration
@EnableWebSecurity
public class SecurityConfig {
    ...
    JwtDecoder jwtDecoder(
            @Value("${wc.auth.mode}") String mode,
            @Value("${wc.auth.audience}") String audience,
            @Value("${wc.auth.real.issuer-uri:#{null}}") String issuerUri,
            @Value("${wc.auth.real.jwk-set-uri:#{null}}") String jwkSetUri,
    ...
    private NimbusJwtDecoder buildMockDecoder(ResourceLoader rl, String audience) throws Exception {
        Resource pem = rl.getResource("classpath:keys/wc-mock-public.pem");
