...
public class ManagerController {
    ...
    /**
     * Returns the current user's direct reports + each report's most-recent plan
     * (or null if they have no plan yet). Capped to a sane page size per the brief's
     * 2000-record requirement; the FE pages through with `?page=&size=&sort=`.
     */
    @GetMapping("/team")
    public Page<TeamMemberDto> team(
            @PageableDefault(size = 25, sort = "displayName", direction = Sort.Direction.ASC) Pageable pageable) {
