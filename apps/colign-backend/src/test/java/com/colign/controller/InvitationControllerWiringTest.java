...
/**
 * End-to-end wiring smoke for the invitation endpoints. Boots the real Spring context (H2 + mock
 * JWT decoder per the test profile) and exercises HTTP + security + DB so that issues
 * SecurityConfig path matching, controller registration, or JSON shape don't slip past the
 * service-layer unit tests in {@link com.colign.service.InvitationServiceTest}.
 *
 * <p>The email client is mocked so a missing Resend key in CI doesn't matter.
 * {@code @DirtiesContext} keeps the per-test H2 instance from leaking rows between this test and
 * any other {@code @SpringBootTest} in the suite.
 */
@SpringBootTest
...
class InvitationControllerWiringTest {
  ...
  void seed() {
    ...
    inviter =
        users
            .findByEmail("lead-wiring@example.com")
            .orElseGet(
                () -> {
                  User u =
                      User.builder()
    ...
    team = (inviter.getTeamId() != null ? teams.findById(inviter.getTeamId()).orElse(null) : null);
    if (team == null) {
      team =
          teams.save(Team.builder().name("Wiring Test Team").leadUserId(inviter.getId()).build());
  ...
  void createInvitation_persistsAndReturnsAcceptUrl() throws Exception {
    ...
    mvc.perform(
            post("/api/v1/teams/" + team.getId() + "/invitations")
                .with(jwtFor(inviter))
    ...
    assertThat(invitations.findAll()).anyMatch(i -> i.getEmail().equals("new-hire@example.com"));
  }

  /**
   * Build a JWT request-post-processor that matches the row {@link #seed()} persisted, so {@code
   * UserResolver.resolveCurrent()} returns it rather than provisioning a new one.
   */
