...
class TeamControllerWiringTest {
  ...
  void seed() {
    ...
    lead =
        users
            .findByEmail("lead-tcw@example.com")
            .orElseGet(
                () ->
                    users.save(
                        User.builder()
                            .email("lead-tcw@example.com")
                            .displayName("Lead")
                            .role(UserRole.IC)
                            .auth0Sub("auth0|lead-tcw")
                            .active(true)
                            .build()));
  ...
  void deleteMember_orphan_reports_and_returns204() throws Exception {
    ...
    User report =
        users
            .findByEmail("report-tcw@example.com")
            .orElseGet(
                () ->
                    users.save(
                        User.builder()
                            .email("report-tcw@example.com")
                            .displayName("Report")
                            .role(UserRole.IC)
                            .auth0Sub("auth0|report-tcw")
                            .active(true)
                            .build()));
    User manager =
        users
            .findByEmail("mgr-tcw@example.com")
            .orElseGet(
                () ->
                    users.save(
                        User.builder()
                            .email("mgr-tcw@example.com")
                            .displayName("Mgr")
                            .role(UserRole.IC)
                            .auth0Sub("auth0|mgr-tcw")
                            .active(true)
                            .build()));
    ...
    mvc.perform(
            delete("/api/v1/teams/" + team.getId() + "/members/" + manager.getId())
                .with(jwtFor(lead)))
  ...
  void deleteMember_400_whenRemovingLead() throws Exception {
    mvc.perform(
            delete("/api/v1/teams/" + team.getId() + "/members/" + lead.getId()).with(jwtFor(lead)))
        .andExpect(r -> assertThat(r.getResponse().getStatus()).isEqualTo(400));
  }
