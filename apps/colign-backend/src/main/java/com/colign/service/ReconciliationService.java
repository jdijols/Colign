...
/**
 * Owns RECONCILING + RECONCILED transitions and the per-commit reconciliation write path.
 * RECONCILED → CARRIED_FORWARD also lives here because the carry-forward step has to atomically
 * inspect this week's commits AND materialize next week's plan.
 */
@Service
public class ReconciliationService {
  ...
  public Plan startReconciliation(Long planId) {
    Plan plan = plans.findById(planId).orElseThrow(() -> NotFoundException.of("Plan", planId));
    if (plan.getState() != PlanState.LOCKED) {
      throw new IllegalTransitionException(
          plan.getState(), PlanState.RECONCILING, "start-reconciliation only valid from LOCKED");
    }
  ...
  public Reconciliation reconcileCommit(Long commitId, ReconcileCommitRequest req) {
    ...
    Reconciliation row =
        existing != null ? existing : Reconciliation.builder().weeklyCommitId(commitId).build();
  ...
  /**
   * Finalize reconciliation: RECONCILING → RECONCILED, then immediately carry forward any MISSED
   * commits into next week's plan (creating that plan in DRAFT if it doesn't exist yet). Returns
   * the now-RECONCILED plan.
   */
  @Transactional
  public Plan finalizeReconciliation(Long planId) {
    ...
    boolean fullyReconciled =
        all.stream().allMatch(c -> reconciliations.findByWeeklyCommitId(c.getId()).isPresent());
    if (!fullyReconciled) {
      throw new IllegalTransitionException(
  ...
  private void carryForward(Plan finalized, List<WeeklyCommit> sourceCommits) {
    List<WeeklyCommit> toCarry =
        sourceCommits.stream().filter(c -> c.getStatus() == CommitStatus.MISSED).toList();
    ...
    Plan nextPlan =
        plans
            .findByUserIdAndWeekStartDate(finalized.getUserId(), nextMonday)
            .orElseGet(
                () ->
                    plans.save(
                        Plan.builder()
