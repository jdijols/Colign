 ...
 *
 * Invariants enforced here (not in the controller, so the unit tests own them):
 * <ul>
 *   <li>Only members of the target team may issue invitations to it.</li>
 *   <li>One pending invite per (email, team) — repeat calls return the existing
 *       row so the recipient never sees two valid links at once.</li>
 *   <li>Inviter cannot invite themselves (no self-promotion to MANAGER by
 *       inviting your own email as REPORT).</li>
 *   <li>Accept requires {@code currentUser.email == invitation.email}
 *       (case-insensitive). Without this anyone with the link is in.</li>
 *   <li>Accept fails if the caller is already on a different team — they must
 *       leave first (out of scope for v1). Same-team is idempotent.</li>
 *   <li>Past {@code expiresAt} the invitation auto-flips to EXPIRED on the
 *       next observation, so the controller surface never has to run a
 *       cleanup job.</li>
 * </ul>
 *
 * Role-leveling is automatic: REPORT acceptance sets
 * {@code invitee.managerId = inviter.id}, which makes
 * {@code UserResolver.derivedRole(inviter)} return MANAGER the next time it
 * runs. Nothing here writes to the inviter's role field.
 */
@Service
public class InvitationService {
    ...
    @Transactional
    public InvitationDto create(Long teamId, String rawEmail, InvitationRelationship relationship, User inviter) {
        Team team = teams.findById(teamId)
                .orElseThrow(() -> NotFoundException.of("Team", teamId));

        if (inviter.getTeamId() == null || !inviter.getTeamId().equals(teamId)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN,
                    "Only members of a team can invite to it");
        }
        ...
        emailClient.sendInvitation(new InvitationEmail(
                email,
    ...
    public InvitationPreviewDto preview(String token) {
        Invitation invitation = invitations.findByToken(token)
                .orElseThrow(() -> NotFoundException.of("Invitation", token));
        Team team = teams.findById(invitation.getTeamId())
                .orElseThrow(() -> NotFoundException.of("Team", invitation.getTeamId()));
        User inviter = users.findById(invitation.getInviterUserId())
    ...
    public MeDto accept(String token, User invitee, UserResolver resolver) {
        ...
        switch (invitation.getStatus()) {
            case ACCEPTED -> throw new ResponseStatusException(HttpStatus.GONE,
                    "This invitation has already been accepted");
            case EXPIRED -> throw new ResponseStatusException(HttpStatus.GONE,
                    "This invitation has expired");
            case REVOKED -> throw new ResponseStatusException(HttpStatus.GONE,
        ...
        if (invitee.getTeamId() != null) {
            throw new ResponseStatusException(HttpStatus.CONFLICT,
                    "You already belong to a different team");
        }
    ...
    public List<InvitationDto> listForTeam(Long teamId, User caller) {
        if (caller.getTeamId() == null || !caller.getTeamId().equals(teamId)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN,
                    "Only members of a team can list its invitations");
        }
        return invitations.findByTeamIdOrderByCreatedDateDesc(teamId).stream()
                .map(inv -> {
                    String inviterName = users.findById(inv.getInviterUserId())
