...
/**
 * Resolves (and lazily provisions) the domain {@link User} for the current JWT principal.
 *
 * <p>Identity key is the Auth0 {@code sub} — stable and always present, unlike email (which arrives
 * via a custom claim and could in principle change). Keying by sub avoids duplicate rows when,
 * e.g., a user was first seen before the profile-claims Action existed (synthetic "<sub>@local"
 * email) and later logs in with their real email.
 *
 * <p>On every resolve we backfill profile fields (displayName / email / avatar) from the current
 * token when it carries better data, so a stale row created before the Auth0 Action was deployed
...
public class UserResolver {
  ...
  /**
   * Namespace for custom claims copied into the ACCESS token by the Auth0 Post-Login Action (OIDC
   * profile fields live only in the ID token by default). Mock-mode tokens put {@code email} at the
   * top level, so every lookup also falls back to the bare claim.
   */
  ...
  public User resolveCurrent() {
    ...
    User existing =
        users.findByAuth0Sub(sub).or(() -> users.findByEmail(resolveEmail(jwt))).orElse(null);
  ...
  private User backfill(User user, Jwt jwt, String sub) {
    ...
    if (email != null && !email.endsWith("@local") && !email.equals(user.getEmail())) {
      user.setEmail(email);
      dirty = true;
    }
  ...
  /**
   * Maps a User to the identity shape the frontend routes on. Shared by GET /me and POST /teams so
   * a single response can re-route the client. Role is the DERIVED role, never the stored field.
   */
  ...
  /**
   * Whether a freshly-created team still needs its first invite. True only when the user is on a
   * team that has exactly one member (them) AND no invitations have been issued yet. As soon as
   * they send one invite, or anyone else joins, this flips false and the onboarding gate lets them
   * into the app. An invited member never sees true — their team already has &gt;1 person by the
   * time they resolve.
   */
  ...
  /**
   * The user's EFFECTIVE role, derived from relationships rather than read from the stored field or
   * the JWT claim: - ADMIN if explicitly flagged ADMIN in the DB (instance operator) - MANAGER if
   * anyone reports to them (managerId == this user) - IC otherwise
   */
  ...
  private String resolveEmail(Jwt jwt) {
    String email = firstNonBlank(jwt.getClaimAsString(NS + "email"), jwt.getClaimAsString("email"));
  ...
  private String resolvePicture(Jwt jwt) {
    return firstNonBlank(jwt.getClaimAsString(NS + "picture"), jwt.getClaimAsString("picture"));
  }
