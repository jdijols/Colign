...
/**
 * Team lifecycle. v1 is deliberately thin: create a team (the onboarding unblock), then invite
 * teammates. The creator becomes the team lead and its first member; they stay IC until their first
 * REPORT invitation is accepted, because role is DERIVED from team relationships (see {@code
 * UserResolver.derivedRole}), not stored.
 */
@RestController
...
public class TeamController {
  ...
  /**
   * Create a team and attach the caller to it as lead. Returns the caller's refreshed identity
   * (same shape as GET /me) so the frontend re-routes straight into the app off a single response —
   * no second round trip.
   *
   * <p>One team per user in v1: if the caller already belongs to a team we 409 rather than silently
   * orphaning their old membership.
   */
  @PostMapping
  @Transactional
  ...
  /**
   * Invite a teammate by email. Any member of the team may invite (the role the inviter takes on
   * the resulting relationship is decided here too — REPORT means "I'll be their manager", PEER
   * means "we're peers"). The email send is synchronous; if it fails we 502 rather than leave a row
   * the recipient never hears about.
   */
  @PostMapping("/{teamId}/invitations")
  public ResponseEntity<InvitationDto> invite(
      @PathVariable Long teamId, @Valid @RequestBody CreateInvitationRequest req) {
    User me = userResolver.resolveCurrent();
  ...
  /**
   * List invitations for the team, newest first. Used by the post-create "invite teammates" screen
   * so the inviter can see pending invites at a glance.
   */
  ...
  @PatchMapping("/{teamId}")
  public TeamDto update(@PathVariable Long teamId, @Valid @RequestBody UpdateTeamRequest req) {
    User me = userResolver.resolveCurrent();
