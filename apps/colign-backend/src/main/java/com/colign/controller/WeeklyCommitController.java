...
public class WeeklyCommitController {
  ...
  public PlanDto add(@PathVariable Long planId, @Valid @RequestBody AddCommitRequest req) {
    ...
    // Server-side strategy gate: a team with no complete RC→DO→Outcome chain cannot author weekly
    // commits yet. The client routing gate is a convenience; this is the enforcement boundary.
    User me = userResolver.resolveCurrent();
    if (!strategyService.isStrategySetupComplete(me.getTeamId())) {
      throw new ResponseStatusException(
          HttpStatus.UNPROCESSABLE_ENTITY,
          "Team strategy setup is incomplete — create a Rally Cry, Objective, and Outcome first");
    }
    if (plan.getState() != PlanState.DRAFT) {
      throw new IllegalTransitionException(
          plan.getState(), plan.getState(), "commits can only be added while the plan is DRAFT");
    }
  ...
  public PlanDto update(@PathVariable Long id, @Valid @RequestBody UpdateCommitRequest req) {
    WeeklyCommit wc =
        commitRepo.findById(id).orElseThrow(() -> NotFoundException.of("WeeklyCommit", id));
  ...
  public PlanDto delete(@PathVariable Long id) {
    ...
    if (plan.getState() != PlanState.DRAFT) {
      throw new IllegalTransitionException(
          plan.getState(), plan.getState(), "commits can only be deleted while the plan is DRAFT");
    }
