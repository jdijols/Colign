...
/**
 * Thin helper for controllers to read the current JWT principal without threading
 * {@code @AuthenticationPrincipal Jwt} through every signature.
 */
public final class CurrentUser {
  ...
  public static List<String> roles() {
    return jwt().map(j -> j.getClaimAsStringList(ROLES_CLAIM)).orElse(Collections.emptyList());
  }
