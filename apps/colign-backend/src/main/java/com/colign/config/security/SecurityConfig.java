 ...
 *
 * <ul>
 *   <li>{@code real} — validate JWTs against the Auth0 tenant configured by {@code
 *       wc.auth.real.issuer-uri} + {@code jwk-set-uri}. This is the intended production path.
 *   <li>{@code mock} — validate JWTs against the local RS256 public key at {@code
 *       classpath:keys/colign-mock-public.pem}, paired with the demo private key in {@code
 *       scripts/colign-mock-private.pem}. Mint tokens via {@code scripts/mock-jwt.mjs}. Demo-only.
 * </ul>
 *
 * Both modes share: 60s clock-skew leeway, audience validation, and the same {@link
 * JwtAuthenticationConverter} (roles read from the namespaced {@code https://colign.org/roles}
 * claim per Auth0 OIDC convention).
 */
@Configuration
@EnableWebSecurity
public class SecurityConfig {
  ...
  /**
   * Comma-separated browser origins allowed to call the API directly. Bound from {@code
   * colign.cors.allowed-origins} (env {@code COLIGN_CORS_ORIGINS} in prod), so the deployed
   * frontend origin doesn't require a code change — the previous hardcoded localhost list was a
   * prod blocker.
   */
        ...
        .authorizeHttpRequests(
            auth ->
                auth.requestMatchers(
                    ...
                    .requestMatchers(
                        org.springframework.http.HttpMethod.GET, "/api/v1/invitations/*")
                    .permitAll()
                    .anyRequest()
                    .authenticated())
  ...
  private NimbusJwtDecoder buildRealDecoder(String issuerUri, String audience) {
    if (issuerUri == null || issuerUri.isBlank()) {
      throw new IllegalStateException("wc.auth.mode=real but wc.auth.real.issuer-uri is empty");
    }
  ...
  JwtAuthenticationConverter jwtAuthenticationConverter() {
    ...
    Converter<Jwt, Collection<GrantedAuthority>> rolesAndScopes =
        jwt -> {
          List<String> roles = jwt.getClaimAsStringList(ROLES_CLAIM);
          Stream<GrantedAuthority> roleAuths =
              roles == null
                  ? Stream.empty()
                  : roles.stream().map(r -> new SimpleGrantedAuthority("ROLE_" + r));
          Collection<GrantedAuthority> scopeAuths = scopes.convert(jwt);
          return Stream.concat(scopeAuths == null ? Stream.empty() : scopeAuths.stream(), roleAuths)
              .toList();
        };
