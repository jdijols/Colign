...
/**
 * Materializes plans + commits + reconciliations for the seeded demo users (Ada, Ben, Chris) the
 * first time the app boots with the V3 seed users present and no commits in the DB. Idempotent on
 * commitRepo.count() — once any commits exist (seeded or created via the API), this runner no-ops.
 *
 * <p>Profile-gated to h2/dev to avoid touching production-shaped Postgres deployments by accident.
 * For a Postgres demo, set SPRING_PROFILES_ACTIVE=demo or temporarily widen the @Profile to include
 * "local".
 */
@Component
@Profile({"h2", "demo"})
public class DemoDataInitializer implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) {
    ...
    outcomes
        ...
        .forEach(
            o ->
                byTier
                    .computeIfAbsent(o.getPriorityTier(), k -> new java.util.ArrayList<>())
                    .add(o));
    if (!byTier.containsKey("P0")
        || byTier.get("P0").size() < 2
        || !byTier.containsKey("P1")
        || byTier.get("P1").size() < 2
        || !byTier.containsKey("P2")
        || byTier.get("P2").isEmpty()) {
    ...
    commit(
        ...
        p1a,
        offense,
        "First-week-active rate funnel review",
    ...
    commit(
        chrisThis,
        chris3.getOutcomeId() == null ? p2.getId() : chris3.getOutcomeId(),
        chris3.getChessTagId(),
        chris3.getTitle(),
        chris3.getDescription(),
        chris3.getPlannedEffortHours(),
        CommitStatus.CARRIED,
        0,
        chris3.getId());

    log.info(
        "DemoDataInitializer seeded: ada DRAFT/2, ben LOCKED/3, chris RECONCILED-last + DRAFT-carried/1");
  }
  ...
  private ChessTag chessTagByCode(String code) {
    return chessTags.findAll().stream()
        ...
        .findFirst()
        .orElseThrow(() -> new IllegalStateException("chess_tag " + code + " not seeded"));
  }
